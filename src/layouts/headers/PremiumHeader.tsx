...
import { Code, User, Settings, LogOut, Crown, Shield, Search, ChevronDown, History, Home, Trophy, Menu, X } from 'lucide-react';
    ...
    <header className="border-b border-border py-4 px-6 bg-gradient-to-r from-gray-900 to-gray-800 text-white">
      <div className="container mx-auto flex justify-between items-center">
        ...
        <nav className="hidden md:flex items-center space-x-6">
          <Button variant="link" className="text-lg text-white hover:text-gray-300" onClick={() => navigate('/')}>
        ...
        <div className="flex items-center space-x-4">
          {isAuthenticated && (
            <Badge variant="default" className="bg-green-500 hover:bg-green-600 text-white px-3 py-1.5 text-sm hidden md:flex">
              {isAdmin ? (
                <>
      ...
      {mobileMenuOpen && (
        <div className="md:hidden mt-4 pb-4">
          <nav className="flex flex-col space-y-4">
            <Button variant="ghost" className="justify-start text-white hover:text-gray-300" onClick={() => { navigate('/'); setMobileMenuOpen(false); }}>
              <div className="flex items-center space-x-2">
                <Home className="h-5 w-5" />
                <span>Home</span>
              </div>
            </Button>
            <Button variant="ghost" className="justify-start text-white hover:text-gray-300" onClick={() => { navigate('/premium-dashboard'); setMobileMenuOpen(false); }}>
              <div className="flex items-center space-x-2">
                <Crown className="h-5 w-5" />
                <span>Premium Dashboard</span>
              </div>
            </Button>
            <Button variant="ghost" className="justify-start text-white hover:text-gray-300" onClick={() => { navigate('/premium/leaderboard'); setMobileMenuOpen(false); }}>
              <div className="flex items-center space-x-2">
                <Trophy className="h-5 w-5" />
                <span>Leaderboard</span>
              </div>
            </Button>
            <Button variant="ghost" className="justify-start text-white hover:text-gray-300" onClick={() => { navigate('/premium/find-match'); setMobileMenuOpen(false); }}>
              <div className="flex items-center space-x-2">
                <Search className="h-5 w-5" />
                <span>Find Match</span>
              </div>
            </Button>
            <Button variant="ghost" className="justify-start text-white hover:text-gray-300" onClick={() => { navigate('/premium/match-history'); setMobileMenuOpen(false); }}>
              <div className="flex items-center space-x-2">
                <History className="h-5 w-5" />
                <span>Match History</span>
              </div>
            ...
            {isAdmin && (
              <Button variant="ghost" className="justify-start text-white hover:text-gray-300" onClick={() => { navigate('/admin'); setMobileMenuOpen(false); }}>
                <div className="flex items-center space-x-2">
                  <Shield className="h-5 w-5" />
                  <span>Admin Panel</span>
                </div>
            ...
            <Button variant="ghost" className="justify-start text-white hover:text-gray-300" onClick={() => { navigate('/premium/settings'); setMobileMenuOpen(false); }}>
              <div className="flex items-center space-x-2">
            ...
            {isAuthenticated && (
              <Badge variant="default" className="bg-green-500 hover:bg-green-600 text-white px-3 py-1.5 text-sm mr-auto ml-2 my-2">
                {isAdmin ? (
                  <>
                    <Shield className="h-5 w-5 mr-1.5" />
                    Admin Access
